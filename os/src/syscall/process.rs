...
pub fn sys_get_time(ts: *mut TimeVal, _tz: usize) -> isize {
    ...
    let token = current_user_token();
    let dest = translated_byte_buffer(token, ts as *const u8, size_of::<TimeVal>());
    let mut start = 0;
    unsafe {
        for buffer in dest {
            core::ptr::copy(src.add(start), buffer.as_mut_ptr(), buffer.len());
            start += buffer.len();
        }
    }
    0
...
pub fn sys_mmap(start: usize, len: usize, prot: usize) -> isize {
    ...
    if frame_remain_num() < (end_va.ceil().0 - start_va.floor().0) {
        return -1;
    }

    current_task()
        .unwrap()
        .insert_framed_area(start_va, end_va, prot << 1 | 16);
