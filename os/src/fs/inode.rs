...
impl File for OSInode {
    fn stat(&self) -> super::Stat {
        let file_inner = self.inner.exclusive_access();
        let mut mode = StatMode::NULL;
        if file_inner.inode.is_dir() {
            mode = StatMode::DIR;
        } else if file_inner.inode.is_file() {
            mode = StatMode::FILE;
        }
        Stat {
            dev: 0,
            ino: file_inner.inode.id() as u64,
            mode: mode,
            nlink: ROOT_INODE.link_num(file_inner.inode.id()),
            pad: [0; 7],
        }
    }
